package com.me.hiet.edit.animatesticker;
...
import com.me.hiet.edit.VideoFragment;
import com.me.hiet.edit.timelineEditor.NvsTimelineEditor;
import com.me.hiet.edit.timelineEditor.NvsTimelineTimeSpan;
import com.me.hiet.edit.view.CustomTitleBar;
import com.me.hiet.edit.view.CustomViewPager;
...
import com.me.hiet.utils.dataInfo.StickerInfo;
...
public class AnimateStickerActivity extends BaseActivity {
    ...
    private int getSelectedPos(){
        ...
        for (int index = 0;index < mTotalStickerAssetList.size();++index){
            if (mTotalStickerAssetList.get(index).uuid.compareTo(mSelectUuid) == 0){
                selectPos = index;
                break;
            }
        }
    ...
    private int getCustomStickerSelectedPos(){
        ...
        for (int index = 0;index < mCustomStickerAssetList.size();++index){
            if (mCustomStickerAssetList.get(index).uuid.compareTo(mSelectUuid) == 0){
                selectPos = index;
                break;
            }
        }
    ...
    private void applyAnimateSticker(int pos){
        ...
        if(mAddAnimateSticker != null && mPrevTabPage == mCurTabPage && mCurSelectedPos == pos){
            isNewStickerUuidItemClick = false;
            if(mVideoFragment.getCurrentEngineState() != NvsStreamingContext.STREAMING_ENGINE_STATE_PLAYBACK){
                mVideoFragment.playVideo(startTime,endTime);
                mVideoFragment.setDrawRectVisible(View.GONE);
        ...
        if(mCurTabPage == 0){//普通贴纸
            mAddAnimateSticker = mTimeline.addAnimatedSticker(mInPoint,mStickerDuration,mTotalStickerAssetList.get(pos).uuid);
        }else {//自定义贴纸
            mAddAnimateSticker = mTimeline.addCustomAnimatedSticker(mInPoint,mStickerDuration,mCustomStickerAssetList.get(pos).templateUuid,mCustomStickerAssetList.get(pos).imagePath);
        }
        if(mAddAnimateSticker == null)
            return;
        mVideoFragment.setDrawRectVisible(View.GONE);
        ...
        mSelectUuid = mCurTabPage == 0 ? mTotalStickerAssetList.get(pos).uuid : mCustomStickerAssetList.get(pos).uuid;
        //保存数据
        ...
        if(mCurTabPage == 1){//存储自定义贴纸数据
            stickerInfo.setCustomSticker(true);
            stickerInfo.setCustomImagePath(mCustomStickerAssetList.get(pos).imagePath);
        }else{
        ...
        timelineTimeSpan.setOnChangeListener(new NvsTimelineTimeSpan.OnTrimOutChangeListener() {
            ...
            public void onChange(long timeStamp, boolean isDragEnd) {
                ...
                if(isDragEnd){
                    ...
                    int index = getAnimateStickerIndex(zVal);
                    if(index >= 0){
                        long duaration = timeStamp - mCurSelectAnimatedSticker.getInPoint();
