...
public class STMobileDetected {
    ...
    private final static String LIENCSE_FILE_NAME = "particle/SenseME.lic";
    private static final String ST_MODEL_NAME_ACTION= "particle/action_5.0.0.model";
    ...
    private STCommon m_STCommon = new STCommon();
    ...
    public boolean initSTMobileDetected(Context context, NvsStreamingContext streamingContext) {
        ...
        streamingContext.setCapturedVideoFrameGrabberCallback(new NvsStreamingContext.CapturedVideoFrameGrabberCallback() {
            @Override
            public void onCapturedVideoFrameGrabbedArrived(ByteBuffer image, NvsVideoFrameInfo frameinfo) {
    ...
    private void stMobileDetected(ByteBuffer ImageData, int width, int height)
    {
        ...
        synchronized (m_detectedObject) {
            humanAction = m_STHumanActionNative.humanActionDetect(ImageData.array(), STCommon.ST_PIX_FMT_NV21,m_detectConfig, faceRotation, width, height);
        }
...
//                    enableParticle(true);
    ...
    private void stMobileDetectedFaceInfo(STMobileFaceInfo info, int width, int height)
    {
        ...
        if(checkDetectedFlag(info.faceAction, STMobileHumanActionNative.ST_MOBILE_MOUTH_AH)){
            ...
            STPoint[] points = arrayFaces.getPoints_array();
    ...
    {
        STPoint[] points = facePoints.getPoints_array();
