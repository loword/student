package com.me.hiet.edit.theme;
    ...
    private void selectThemeCaption(){
        ...
        if(captionCount > 0){
            for(int i = 0; i < captionCount; i++){
                ...
                if(caption != null){
                    int themeCaptionCate = caption.getCategory();
                    Logger.e(TAG,"themeCaptionCate = " + themeCaptionCate);
                    int roleCapion = caption.getRoleInTheme();
                    Logger.e(TAG,"roleCapion = " + roleCapion);
                }
                if(caption.getCategory() == NvsTimelineCaption.THEME_CATEGORY
                        && caption.getRoleInTheme() != NvsTimelineCaption.ROLE_IN_THEME_GENERAL) {//查找主题字幕
                    mThemeCaption = captionList.get(i);
                    break;
                }
            }
        }else {
