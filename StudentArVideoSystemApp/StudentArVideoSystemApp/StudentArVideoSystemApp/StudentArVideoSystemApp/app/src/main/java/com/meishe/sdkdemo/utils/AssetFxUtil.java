...
public class AssetFxUtil {
    ...
    //获取人脸道具数据选中位置
    public static int getSelectedFaceUPropPos(ArrayList<FilterItem> faceUDataArrayList, String curFaceUName) {
        if (faceUDataArrayList == null || faceUDataArrayList.size() == 0)
            return -1;

        if (TextUtils.isEmpty(curFaceUName))
            return 0;
        ...
        for (int i = 0; i < faceUDataArrayList.size(); i++) {
            ...
            if (newFilterItem == null)
                continue;
            String faceUName = newFilterItem.getFilterName();
            if (curFaceUName.equals(faceUName)) {
                return i;
            }
        }
