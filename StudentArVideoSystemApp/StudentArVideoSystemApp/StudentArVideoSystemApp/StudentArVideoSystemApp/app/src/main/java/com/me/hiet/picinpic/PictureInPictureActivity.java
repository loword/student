...
import com.me.hiet.picinpic.data.PicInPicDrawRect;
import com.me.hiet.picinpic.data.PicInPicTemplateAsset;
...
//        });
    ...
    private void initTrackClip(){
        ...
        if(mFilePathList == null || mFilePathList.size() == 0)
            return;
        addVideoClipByFirst(mFilePathList, 0, -1, 0, -1);
        //获取裁剪点值
        setTrimPointValue();
        //添加剩余视频片段
        addRemainVideoClip(mFilePathList);
    ...
    private void updateTrackClip(){
        ...
        if(mFilePathList == null || mFilePathList.size() == 0)
            return;
        addVideoClipByFirst(mFilePathList,
                mFirTrimInPoint,
                mFirTrimOutPoint,
                mSecTrimInPoint,
                mSecTrimOutPoint
    ...
    private void  removeAllClips(){
        if(mVideoTrackFir.getClipCount() > 0)
            mVideoTrackFir.removeAllClips();
        if(mVideoTrackSec.getClipCount() > 0)
            mVideoTrackSec.removeAllClips();
    }

    //第一次添加片段
    private void addVideoClipByFirst(ArrayList<String> picInPicArray,
                                long firTrackTrimIn,
                                long firTrackTrimOut,
                                long secTrackTrimIn,
                                long secTrackTrimOut){
        addVideoClip(mVideoTrackFir,
                picInPicArray.get(0),
                firTrackTrimIn,
                firTrackTrimOut
        );

        addVideoClip(mVideoTrackSec,
                picInPicArray.get(1),
                secTrackTrimIn,
                secTrackTrimOut
        );
    ...
    //计算把某一轨道剩余视频片段补齐
    private void addRemainVideoClip(ArrayList<String> picInPicArray){
    ...
    private void addVideoClip(NvsVideoTrack videoTrack,
                              String clipPath,
                              long needAddClipCount,
                              long remainedClipDuration,
                              long trimIn,
                              long trimOut){
        while (needAddClipCount > 0){
            addVideoClip(videoTrack,clipPath,trimIn,trimOut);
            --needAddClipCount;
        }
        if(remainedClipDuration > 0){
            addVideoClip(videoTrack,clipPath,trimIn,trimIn + remainedClipDuration);
        }
    }
