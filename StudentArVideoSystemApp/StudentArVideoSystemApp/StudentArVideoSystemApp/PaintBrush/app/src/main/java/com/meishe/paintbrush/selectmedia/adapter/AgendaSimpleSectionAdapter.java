...
import com.meishe.paintbrush.selectmedia.SelectMediaActivity;
import com.meishe.paintbrush.selectmedia.bean.MediaData;
import com.meishe.paintbrush.selectmedia.interfaces.OnItemClick;
import com.meishe.paintbrush.selectmedia.interfaces.OnTotalNumChange;
...
public class AgendaSimpleSectionAdapter extends SimpleSectionedAdapter<AgendaItemViewHolder> implements OnItemClick {
    ...
    public AgendaSimpleSectionAdapter(List<List<MediaData>> agenda, List<MediaData> listOfOut, RecyclerView recyclerView,
                                         OnTotalNumChange onTotalNumChange, int tag, Activity mActivity,int clickType,int limitCount) {
    ...
    protected String getSectionHeaderTitle(int section) {
        ...
        return todayTime.equals(listOfParent.get(section).getDate()) ? "今天" : listOfParent.get(section).getDate();
    }
    ...
    public void OnItemClick(View v, int headListPosition, int childListPosition) {
        if (!lists.get(headListPosition).get(childListPosition).isState() && limitCount != -1 && SelectMediaActivity.getTotal() == limitCount){
            return;
        }
    ...
    public void itemClick(View v, int headListPosition, int childListPosition, boolean changeTotal) {
        ...
        if (clickType == MediaConstant.TYPE_ITEMCLICK_SINGLE){
            onSingleClick(lists.get(headListPosition).get(childListPosition).isState(), headListPosition, childListPosition, recyclerView.getChildAdapterPosition(v));
        }else {
            //改变总数的值
    ...
    private void onSingleClick(boolean isAdd, int headListPosition, int childListPosition, int positionInRecycle){
    ...
    private void totalChange(boolean isAdd, int headListPosition, int childListPosition, int positionInRecycle, boolean changeTotal) {
        ...
        if (!isAdd) {
            if (changeTotal) {
                SelectMediaActivity.setTotal(SelectMediaActivity.getTotal() + 1);
            }
            selectList.add(lists.get(headListPosition).get(childListPosition));
        ...
        lists.get(headListPosition).get(childListPosition).setPosition(SelectMediaActivity.getTotal());
        recyclerView.getAdapter().notifyItemChanged(positionInRecycle);
    }

    private void onTotalChange() {
        if (mActivity != null && selectList != null) {
            if (mActivity instanceof SelectMediaActivity){
                SelectMediaActivity activity = (SelectMediaActivity) mActivity;
                activity.setTitleText(selectList.size());
            }
        }
